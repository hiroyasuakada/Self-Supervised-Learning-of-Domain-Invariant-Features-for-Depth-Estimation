...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # self.netG_s2t = network.define_G(opt.image_nc, opt.image_nc, opt.ngf, opt.transform_layers, opt.norm,
        #                                 opt.activation, opt.trans_model_type, opt.init_type, opt.drop_rate,
        #                                 False, opt.gpu_ids, opt.U_weight)
        # self.netG_t2s = network.define_G(opt.image_nc, opt.image_nc, opt.ngf, opt.transform_layers, opt.norm,
        #                                 opt.activation, opt.trans_model_type, opt.init_type, opt.drop_rate,
        #                                 False, opt.gpu_ids, opt.U_weight)
    ...
    def test(self):
        ...
        with torch.no_grad():
            # self.img_s2t = self.netG_s2t.forward(self.img_s)
            # self.img_t2s = self.netG_t2s.forward(self.img_t)
            self.lab_t_g = self.netTask.forward(self.img_t)
    ...
    def save_results(self, visualizer, wed_page):
        ...
        for i in range(self.img_s.size(0)):
            ...
            if self.opt.pre_trained_on_ImageNet is False:
                ...
                # img_target2source = util.tensor2im(self.img_t2s[-1].data[i])
                lab_fake_target = util.tensor2im(self.lab_t_g[-1].data[i])
            else:
                # img_source = util.tensor2im(self.img_s.data[i], denorm_ImageNet=True)
                # img_target = util.tensor2im(self.img_t.data[i], denorm_ImageNet=True)
                # img_source2target = util.tensor2im(self.img_s2t[-1].data[i], denorm_ImageNet=True)
                # img_target2source = util.tensor2im(self.img_t2s[-1].data[i], denorm_ImageNet=True)
                lab_fake_target = util.tensor2im(self.lab_t_g[-1].data[i], depth=True)
            ...
            # visuals = OrderedDict([('img_t', img_target), ('img_t2s', img_target2source), ('lab_t_g', lab_fake_target)])
