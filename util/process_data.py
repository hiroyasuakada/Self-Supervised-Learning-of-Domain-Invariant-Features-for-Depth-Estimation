...
def compute_errors(ground_truth, predication):
    ...
    # Log_10
    log_10 = (np.abs(np.log10(ground_truth)-np.log10(predication))).mean()
    ...
    return abs_rel, sq_rel, rmse, rmse_log, log_10, a1, a2, a3

def load_depth(file_path, split, normalize_depth):
    depths = []
    dataset, _ = make_dataset(None, file_path)
    for data in dataset:
        depth = Image.open(data)
        depth = np.array(depth)#[:,:,0]
        depth = depth.astype(np.float32) / 255 * normalize_depth
